...
$Folder_Report = "WINDOWS_LIVEFORENSICS_REPORT-$Date"
...
gp -ea 0 'hkcu:\software\Microsoft\Windows\CurrentVersion\run' | select * -ExcludeProperty PS* | ConvertTo-html -Body "<H2> Startup Applications - Generic / User </H2>" >> $OutLevel1

gp -ea 0 'hklm:\software\Microsoft\Windows\CurrentVersion\run' | select * -ExcludeProperty PS* | ConvertTo-html -Body "<H2> Startup Applications - Generic / User </H2>" >> $OutLevel1
...
gwmi -ea 0 win32_Service | select Name,parentprocessid,ProcessId,State,DisplayName,PathName | sort state | ConvertTo-html -Body "<H2> Running Services - Sorted by State </H2>" >> $OutLevel1


Write-host "[+] - Extracting  - Running Services - Sorted by Name"
gwmi -ea 0 win32_Service | select Name,parentprocessid,ProcessId,State,DisplayName,PathName | sort name| ConvertTo-html -Body "<H2> Running Services - Sorted by  Name </H2>" >> $OutLevel1
